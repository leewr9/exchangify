...
def fetch_chart_data(start_date, end_date, search_unit=None):
    try:
        ...
        exchanges = Exchange.objects.filter(
            cur_date__gte=start_date, cur_date__lte=end_date
        )
...
def fetch_unit_data(start_date, end_date, search_unit):
    ...
    exchanges = Exchange.objects.filter(
        cur_unit=search_unit, cur_date__gte=start_date, cur_date__lte=end_date
    ).order_by("cur_date")
    print(exchanges)
    ...
    avg_deal_bas_r = (
        round(aggregated_data["avg_deal_bas_r"], 2)
        if aggregated_data["avg_deal_bas_r"]
        else 0
    )
